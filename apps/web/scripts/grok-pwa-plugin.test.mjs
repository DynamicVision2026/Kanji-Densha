...
/**
 * A workspace with no src/lib/og/site.json or public/og.jpg. Any call that
 * omits `cwd` defaults to `process.cwd()` — this repo's real workspace,
 * which ships its own site.json (title "漢字でんしゃ") and public/og.jpg —
 * so a test asserting a synthetic title or the placeholder card must pass
 * one of these or it silently asserts against the real product identity
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, { appName: "Wild Race", cwd: isolatedCwd() });
...
test("a cwd card file wins even over an explicit site without card=custom (stale-bake safety net)", () => {
  // This disagreed with normalizeHeadContext's own behavior and doc comment
  // ("Still consult the workspace so a public/og.jpg generated after that
  // snapshot — or missed by a wrong cwd — wins over the og.grok.me
  // placeholder") since both landed in the same harvest commit; the
  // assertions here were never actually true. The re-check is deliberate:
  // Cloud Run serves from disk at runtime (unlike Vercel, which has no
  // public/ to read), so a stale or wrong-cwd bake can't silently ship the
  // placeholder over a real custom card that is actually sitting on disk.
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({ appName: "Wild Race", cwd: isolatedCwd() });
  const chunks = [
