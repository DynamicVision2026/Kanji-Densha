...
| TTS audio (future generation) | VOICEVOX + VOICEVOX Nemo voice library, generated locally, self-hosted | Free for commercial use **with credit** | Credit token `VOICEVOX:Nemo` verbatim; must not present as human-recorded; no training/fine-tuning proprietary models on the output without separate licence | Parent credits / legal page | 2026-08-26 |
| TTS audio (1,121 elementary-reading clips, retained, D23) | xAI TTS API (`api.x.ai/v1/tts`), voice `eve`, generated by the harvested Dynamic-Densha codebase | Cleared on ownership, 2026-08-24 review (per founder) | Standard xAI credit line on parent credits page; no separate per-file terms archive exists — see note below | Parent credits page | 2026-08-28 (D23) |
...
The prior xAI TTS decision is withdrawn **for new generation**. Do not use it, and do not
request or store an `XAI_API_KEY`, for anything generated from here forward — word-surface
audio included.

**D23 (2026-08-28) — the existing 1,121 xAI clips are retained, not regenerated.** The
withdrawal above was a forward-looking vendor choice, not a finding that xAI's terms failed;
the founder's 2026-08-24 review cleared xAI on ownership, and that clearance covers files
already generated before the VOICEVOX decision. These are the harvested Dynamic-Densha
reading-audio files (engine `xai-tts`, voice `eve` — one consistent voice across all of them,
which is the property that matters to a child). No generation manifest or per-file parameters
exist for this batch, unlike the VOICEVOX pilot's `audio_query` records — nothing to hand a
reviewer for exact regeneration if an accent correction is ever needed; a flagged file would be
regenerated from scratch rather than patched. VOICEVOX remains the engine for all *future*
generation, word-surface audio included.
